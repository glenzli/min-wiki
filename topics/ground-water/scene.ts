...
export function draw(p:number,condition:number){
 ...
 const infiltrated=Array.from({length:Math.round(water.soaked/2)},(_,i)=>`<circle cx="${101+(i%14)*45}" cy="${309+Math.floor(i/14)*36+(i%3)*9}" r="4" fill="#258fb2"/>`).join('');
 const rain=Array.from({length:20},(_,i)=>`<path d="M${154+(i%10)*42} ${143+Math.floor(i/10)*49}l-8 20" stroke="#68b8cb" stroke-width="3" stroke-linecap="round" opacity="${stage<2?.8:.2}"/>`).join('');
 const storedHeight=water.stored*.7;
 const arrows=Array.from({length:condition===0?6:condition===1?3:1},(_,i)=>`<path d="M${160+i*76} 275v${20+water.soaked*.7}" stroke="#368ba8" stroke-width="3" stroke-dasharray="5 5" marker-end="url(#arrow)" opacity="${p>.25?.65:0}"/>`).join('');
 ...
 ${infiltrated}${arrows}<path d="M560 257Q671 259 757 289V399" fill="none" stroke="#4da3bb" stroke-width="${2+water.flowed/8}" opacity="${p>.5?.8:0}" marker-end="url(#arrow)"/>
