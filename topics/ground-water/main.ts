...
function update(){
 ...
 el('metrics').replaceChildren(...result.labels.map(label=>{const span=document.createElement('span');span.textContent=label;span.style.fontVariantNumeric='tabular-nums';return span;}));
}
