...
export function draw(p:number,condition:number){
 ...
 <path d="${curve}" fill="none" stroke="#c2ede1" stroke-width="5" stroke-linecap="round" pathLength="100" stroke-dasharray="3 11" stroke-dashoffset="${-p*20}" opacity=".7"/>
 ${sediment}${rock}
 ...
 ${`<g opacity="${Math.max(0,Math.min(1,(p-.65)/.25))}">${txt(outerLabel,196,t('外侧冲刷'))}<path d="M${outerLabel+(outerRight?-66:66)} 204L${outer[0]} ${outer[1]}" stroke="#a17441" stroke-width="2"/>${txt(innerLabel,324,t('内侧沉积'))}<path d="M${innerLabel+(outerRight?66:-66)} 316L${inner[0]} ${inner[1]}" stroke="#a17441" stroke-width="2"/></g>`}`};
}
