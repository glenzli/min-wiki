...
document.querySelectorAll<HTMLButtonElement>('[data-collapse]').forEach(b => b.addEventListener('click', () => { goTo(Number(b.dataset.collapse)); el('scene').scrollIntoView({ block: 'center', behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); }));
