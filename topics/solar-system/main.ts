...
import { SolarSimulation } from './physics/solarSimulation.ts';
...
interface PageElements {
  ...
  'model-year': HTMLElement;
  'model-kepler': HTMLElement;
  'model-scale': HTMLElement;
  'story': HTMLElement;
  ...
  'planet-strip': HTMLElement;
  'canvas-container': HTMLElement;
  'scene-status': HTMLElement;
  'planet-card': HTMLElement;
  'card-name': HTMLElement;
  'card-type': HTMLElement;
  'btn-close-card': HTMLButtonElement;
  'card-diameter': HTMLElement;
  'card-ratio': HTMLElement;
  'card-period': HTMLElement;
  'card-rotation': HTMLElement;
  'card-fact': HTMLElement;
...
class SolarApp {
  ...
  planetLabels!: {data: Planet; label: HTMLSpanElement}[];
  frame!: number;
  ...
  constructor() {
    ...
    document.querySelectorAll<HTMLElement>('[data-view]').forEach(b => { b.hidden = ['lineup','planets'].includes(b.dataset.view!); });
  ...
  initPlanetStrip() {
    ...
    for (const p of PLANETS_DATA) {
      ...
      btn.innerHTML = `<span class="p-dot" style="background:${p.colorHex}"></span>${p.displayName}`;
      btn.setAttribute('aria-label', t("聚焦{{v0}}", {v0: p.displayName}));
  ...
  bindControls() {
    ...
    $('planet-strip').addEventListener('click', event => {
      const btn = (event.target as HTMLElement).closest<HTMLElement>('.planet-tab');
      if (!btn) return;
  ...
  selectScenario(sc: string) {
    ...
    document.querySelectorAll<HTMLElement>('.planet-tab').forEach(b => b.classList.toggle('active',!b.dataset.planet!));
    document.querySelectorAll<HTMLElement>('[data-view]').forEach(b => {
  ...
  selectPlanet(planetId: string) {
    ...
    document.querySelectorAll<HTMLElement>('[data-view]').forEach(b => b.setAttribute('aria-pressed','false'));
  }

  refreshPlanetCard() {
    const data = PLANETS_DATA.find(p => p.id === this.selectedPlanet);
    if (!data) return;
    $('card-name').textContent = data.displayName;
    $('card-type').textContent = data.typeZh;
    $('card-diameter').textContent = `${data.diameterKm.toLocaleString()} km`;
    $('card-ratio').textContent = t("{{v0}} × 地球直径", {v0: (data.diameterKm / 12742).toFixed(2)});
    $('card-period').textContent = t("{{v0}} 天（{{v1}} 年）", {v0: data.orbitalPeriodDays.toLocaleString(), v1: data.orbitalPeriodYears});
    $('card-rotation').textContent = t("{{v0}} 小时{{v1}}", {v0: Math.abs(data.rotationHours), v1: data.rotationHours<0?t("（逆行）"):''});
  ...
  syncPlayButton() {
    ...
    $('play-text').textContent = playing ? t("暂停公转") : t("继续公转");
    $('btn-play').setAttribute('aria-label', playing ? t("暂停公转") : t("继续公转"));
  }
  ...
  syncScrubber() {
    ...
    $('elapsed').textContent = t("{{v0}} 年", {v0: this.simulation.timeYears.toFixed(1)});
  }
  ...
  updateStory() {
    ...
    if (this.mode === 'academic') {
      $('model-year').textContent = t("{{v0}} 年", {v0: this.simulation.timeYears.toFixed(2)});
      const planet = PLANETS_DATA.find(p => p.id === this.selectedPlanet) ?? PLANETS_DATA[2];
      $('model-kepler').textContent = `${(planet.semiMajorAxisAU**3 / planet.orbitalPeriodYears**2).toFixed(4)}（${planet.displayName}）`;
      $('model-scale').textContent = this.scenario === 'lineup' ? t("直径同尺度") : t("轨道间距压缩");
    }
  }
