...
const urls={mercury,venus,earth,mars,jupiter,saturn,uranus,neptune,moon};
...
export class PlanetMaterials {
 constructor(renderer) {this.loader=new THREE.TextureLoader();this.textures=new Set();this.disposed=false;this.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());}
 load(url,ready,color=true) {
 ...
 planet(data) {
  const uniforms={uMap:{value:null},uReady:{value:0},uCloudMap:{value:null},uCloudReady:{value:0},uBaseColor:{value:new THREE.Color(data.colorHex)},uLightPos:{value:new THREE.Vector3()},uComparison:{value:0},uTime:{value:0},uAtmosphere:{value:data.id==='earth'?1:['venus','uranus','neptune'].includes(data.id)?.25:0}};
