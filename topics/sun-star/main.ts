...
el('distance').addEventListener('input',update);el('near').addEventListener('click',()=>{(el('distance') as HTMLInputElement).value='0';update();});el('far').addEventListener('click',()=>{(el('distance') as HTMLInputElement).value='3';update();});update();
