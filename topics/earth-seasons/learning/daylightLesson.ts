...
function text(ctx: CanvasRenderingContext2D,value: string,x: number,y: number,size=12,color='#b1bfd0',align: CanvasTextAlign='center') {
 ctx.font=`${size}px system-ui, sans-serif`; ctx.fillStyle=color;ctx.textAlign=align;ctx.fillText(value,x,y);
}
function circle(ctx: CanvasRenderingContext2D,x: number,y: number,r: number,color: string | CanvasGradient) {ctx.beginPath();ctx.arc(x,y,r,0,TAU);ctx.fillStyle=color;ctx.fill();}
...
function draw(canvas: HTMLCanvasElement,model: ReturnType<typeof daylightExperiment>,progress: number) {
 ...
 text(ctx,t("阳光从左边来"),78,46,10,'#8092ac');
 text(ctx,t("黄色越长，白天越长"),cx,46,10,'#8092ac');
 ...
 text(ctx,t("北"),gcx+ax*gr*1.52,gcy-ay*gr*1.52,10,'#d5e7ea');
 for(let j=0;j<120;j++){
 ...
 text(ctx,t("城市所在的纬度圈"),78,227,10,'#b1bfd0');
 ...
 circle(ctx,cx,cy,3,'#889eb8');text(ctx,t("地轴"),cx,cy+19,10);
 text(ctx,state.state==='day'?t("城市在亮面里 · 白天"):state.state==='night'?t("城市在暗面里 · 黑夜"):t("城市在晨昏交界"),cx,245,10,state.state==='day'?'#f6ce77':'#9bb6dd');
 ctx.strokeStyle='#26384d';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(297,18);ctx.lineTo(297,252);ctx.stroke();
 text(ctx,t("正午，太阳有多高？"),400,26,13,'#e3e9f0');
 ...
 text(ctx,t("当地地面"),421,gy+20,10,'#aeb9ad');
 ...
 } else {circle(ctx,sx,Math.min(233,sy),9,'#516787');text(ctx,t("太阳低于地平线"),403,76,11,'#9bb6dd');}
...
export function mountDaylightLesson(host: HTMLElement) {
 ...
 function refresh(){
  ...
  host.querySelectorAll<HTMLElement>('[data-orbit]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.orbit)===orbit)));
  cards.forEach((card,i)=>{
   const m=models[i],night=m.daylight===null?null:24-m.daylight;
   card.querySelector<HTMLElement>('small')!.textContent=t("倾角 {{v0}}°", {v0: m.tilt.toFixed(2)});
   card.querySelector<HTMLElement>('[data-hours]')!.textContent=m.daylight===null?t("太阳在地平线附近"):t("白昼 {{v0}}", {v0: hours(m.daylight)});
   card.querySelector<HTMLElement>('[data-altitude]')!.textContent=t("正午太阳高度 {{v0}}°", {v0: m.noonAltitude.toFixed(1)});
   card.querySelector<HTMLElement>('.lesson-daybar')!.style.background=m.daylight===null?'#55606c':'#3e5c83';
   card.querySelector<HTMLElement>('.lesson-daybar span')!.style.width=m.daylight===null?'0%':`${m.daylight/24*100}%`;
   card.querySelector<HTMLElement>('[data-day-label]')!.textContent=m.daylight===null?t("极点的特殊情况"):t("白昼 {{v0}}", {v0: hours(m.daylight)});
   card.querySelector<HTMLElement>('[data-night-label]')!.textContent=night===null?t("普通昼长公式不适用"):t("黑夜 {{v0}}", {v0: hours(night)});
   card.querySelector<HTMLCanvasElement>('canvas')!.setAttribute('aria-label',t("{{v0}}，倾角 {{v1}} 度；白昼 {{v2}}，正午太阳高度 {{v3}} 度。黄色为日侧路程，蓝色为夜侧路程。", {v0: places.find(p=>p.lat===latitude)!.name, v1: m.tilt.toFixed(2), v2: hours(m.daylight), v3: m.noonAltitude.toFixed(1)}));
  });
  const m=models[1],place=latitude>0?t("北半球"):latitude<0?t("南半球"):t("赤道");
  host.querySelector<HTMLElement>('[data-conclusion]')!.textContent=Math.abs(latitude)===90 && m.daylight!==null ? (m.daylight>12?t("北极整天都在阳光里：极昼。"):t("北极整天都在黑夜里：极夜。")) : daylightDifference(m);
  let reason;
  if(m.daylight===null)reason=t("极点不画出普通城市的日出日落：太阳始终沿理想地平线运动。试试北京，更容易比较黄色路段的长短。");
  else if(Math.abs(latitude)===90)reason=t("这里的纬度圈已经缩成一个点；大圆只表示一天的时间。{{v0}}极点在不倾斜时是地平线退化情况，不能用 12 小时作为比较基准。", {v0: m.daylight>12?t("整圈都是黄色，表示太阳整天都在地平线上方。"):t("整圈都是蓝色，表示太阳整天都在地平线下方。")});
  else if(tilt<.1)reason=t("没有倾角时，城市每天走的圈有一半在亮面里、一半在暗面里。把右边的滑块慢慢拉向 23.44°，观察黄色路段和正午太阳。");
  else if(Math.abs(latitude)<.1)reason=t("赤道很特别：这个模型里白昼全年约 12 小时，但正午太阳高度仍会变化。换到北京或悉尼，白昼长短的变化会更明显。");
