...
export function surfaceNormal(latitude: number, longitude: number) {
...
export function solarGeometry(orbitProgress: number, rotationProgress: number, tiltDeg: number) {
...
export function cityIllumination(latitude: number, longitude: number, sunLocal: {x:number;y:number;z:number}) {
