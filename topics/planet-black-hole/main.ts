...
const positions = STAGE_PROGRESS;
function update() {
    const busy = scene?.loading ?? false;
    el('scene-loading').hidden = !busy;
    el('scene-error').hidden = !!scene && !scene.error;
    el('scene').setAttribute('aria-busy', String(busy));
    (el('play') as HTMLButtonElement).disabled = !scene?.data;
    const data = CONTENT[route], shared = CONTENT.shared, stage = Math.max(0, positions.reduce<number>((found, p, i) => progress >= p ? i : found, 0));
  ...
  for (const key of ['title', 'formula', 'terms', 'watch', 'caution'] as const) el(`science-${key}`).textContent = science[key];
    ...
    const duration = playbackDuration(planet, route), elapsed = playbackSeconds(progress, planet, route);
    (el('progress') as HTMLInputElement).value = String(Math.round(elapsed / duration * 1000));
    el('progress').setAttribute('aria-valuetext', t('演示时间 {{elapsed}} / {{duration}} 秒（1×）', { elapsed: elapsed.toFixed(1), duration }));
    el('elapsed').textContent = `${elapsed.toFixed(1)} / ${duration} s`;
    el('playback-note').textContent = duration > 32
        ? t('1× 下演示约 2 分钟，碎屑回返时会慢下来。点“稍后”再播放，还能观察约 1 分钟；随时暂停，换个角度看看。')
        : t('1× 下完整飞掠约 32 秒。拖动进度或换一颗行星，可以停在同一位置比较。');
...
el('progress').addEventListener('input', () => { progress = motionProgress(Number((el('progress') as HTMLInputElement).value) / 1000 * playbackDuration(planet, route), planet, route); playing = false; update(); });
...
function animate(now: number) {
    ...
    last = now;
    if (!document.hidden) {
        const preparation = `${scene?.loading}:${scene?.error}`;
        if (preparation !== lastPreparation) { lastPreparation = preparation; update(); }
        if (playing && scene?.data) {
            progress = advancePlayback(progress, dt * Number((el('speed') as HTMLSelectElement).value), planet, route);
            if (progress >= 1)
                playing = false;
            update();
        }
