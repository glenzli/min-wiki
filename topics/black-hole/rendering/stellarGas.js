...
import { CORE_RELEASE_END } from '../physics/stellarDisruption.js';
...
export class StellarGas {
  ...
  setModel(model) {
    ...
    for (const [attribute, data] of [['origin', model.initial], ['variation', model.variation], ['bound', model.bound]]) {
  ...
  update(progress) {
    const attributes = this.points.geometry.attributes;
    const focus = this.model.sample(progress, attributes.position.array, attributes.previous.array, attributes.state.array);
