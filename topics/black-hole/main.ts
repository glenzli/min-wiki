...
import { Playback, SCENARIOS, orbitAt, TIDAL_RADIUS, motionProgress, playbackProgress } from './physics/encounter.ts';
import { STORIES, ACADEMIC, stageAt, ROUTE_DESCRIPTIONS } from './story.ts';
import { SpaceSynth } from './audio/spaceSynth.ts';

interface PageElements {
  'encyclopedia-nav': HTMLElement;
  'btn-about': HTMLButtonElement;
  'model-distance': HTMLElement;
  'model-speed': HTMLElement;
  'model-beta': HTMLElement;
  'story': HTMLElement;
  ...
  'gas-outcome': HTMLElement;
  'view-note': HTMLElement;
  'scene-loading': HTMLElement;
  'gas-error': HTMLElement;
  'btn-retry-gas': HTMLButtonElement;
  'star-label': HTMLElement;
  'hole-label': HTMLElement;
  'scene-legend': HTMLElement;
  'tidal-legend': HTMLElement;
  ...
  'toggle-sound': HTMLInputElement;
  'observation-note': HTMLElement;
...
class App {
  ...
  bindControls() {
    document.querySelectorAll<HTMLElement>('[data-mode]').forEach(button => {
      ...
      button.addEventListener('click', () => {
        ...
        document.querySelectorAll<HTMLElement>('button[data-mode]').forEach(b => b.setAttribute('aria-pressed', String(b === button)));
        this.updateUI();
      });
    });
    ...
    document.querySelectorAll<HTMLElement>('[data-scenario]').forEach(button => button.addEventListener('click', () => {
    ...
    $('speed').addEventListener('change', event => { this.playback.speed = Number((event.target as HTMLInputElement).value); });
    ...
    document.querySelectorAll<HTMLElement>('[data-view]').forEach(button => button.addEventListener('click', () => {
      ...
      document.querySelectorAll<HTMLElement>('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));
    }));
    $('toggle-guides').addEventListener('change', event => {
      if(this.simulation) this.simulation.guidesVisible=(event.target as HTMLInputElement).checked;
      $('scene-legend').hidden=!(event.target as HTMLInputElement).checked;
    });
    $('toggle-sound').addEventListener('change', event => { this.soundEnabled=(event.target as HTMLInputElement).checked; this.syncAudio(); });
    ...
    document.addEventListener('keydown', event => {
      if(event.code!=='Space'||event.repeat||(event.target as HTMLElement).closest('button,input,select,textarea,a,[contenteditable]')||this.dialog.open) return;
  ...
  buildStages() {
    ...
    document.querySelectorAll<HTMLElement>('[data-scenario]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.scenario===scenario)));
  ...
  updateUI() {
    ...
    const outcome = this.mode==='academic'
      ? t("已跨过模型内界：{{v0}}% 的气体采样点", {v0: (absorbed/12000*100).toFixed(1)})
      : t("一部分气体已落入黑洞，外面的气体还在运动");
    ...
    if(this.phaseKey!==key) {
      ...
      $('notice-heading').textContent=academic?t("可以这样讲给孩子听"):t("找一找，想一想");
    ...
    if(this.mode==='academic') {
      ...
      $('model-distance').textContent=absent?t("无黑洞"):intact?(p.radius/TIDAL_RADIUS).toFixed(2):t("已瓦解");
    ...
    const label=playing?t("暂停看看"):progress>=1?t("再看一次"):progress>0?t("继续旅行"):t("开始旅行");
  ...
  updateLabels() {
    ...
    const place=(element: HTMLElement,point: ReturnType<TdeSimulation["labelPosition"]>,dx: number,dy: number,visible: boolean)=>{
