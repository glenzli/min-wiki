...
let reserve=2,night=false,shownReserve=2;
let cancelReserve: () => void = () => {};
function setReserve(value:number) {
  cancelReserve(); reserve=value;
  cancelReserve=animateValue({from:shownReserve,to:value,duration:1300,onUpdate:current=>{shownReserve=current;draw();}});
}
el('rain').addEventListener('click',()=>setReserve(3));
el('dry').addEventListener('click',()=>setReserve(Math.max(0,reserve-1)));
el('day').addEventListener('click',()=>{night=false;draw()});
el('night').addEventListener('click',()=>{night=true;draw()});
el('layer').addEventListener('change',draw);

function draw() { const skin=el<HTMLSelectElement>('layer').value==='skin';const width=58+shownReserve*11;
