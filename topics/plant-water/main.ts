...
el('step').addEventListener('input',draw);el('next').addEventListener('click',()=>{const step=el<HTMLInputElement>('step');step.value=String(Math.min(3,Number(step.value)+1));draw()});el('reset').addEventListener('click',()=>{el<HTMLInputElement>('step').value='0';draw()});
...
const points=[[259,440],[375,318],[438,151],[517,73]];const point=points[s]!;art+=`<circle cx="${point[0]}" cy="${point[1]}" r="15" fill="#43b2d5" stroke="white" stroke-width="4"/>`;
