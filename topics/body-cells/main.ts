...
function draw(){
  const focus=kind==='neuron'?{x:824,y:268,w:300}:kind==='muscle'?{x:490,y:361,w:710}:{x:490,y:249,w:560};
  ...
  scene.innerHTML=renderCell(kind,progress,t);
...
function setup(){
  ...
  info.stages.forEach((item,index)=>{const button=document.createElement('button');button.type='button';button.dataset.step=String(index);button.textContent=item.title;
    button.addEventListener('click',()=>{stop();cancelSwitch();scene.style.opacity='1';const target=(kind==='barrier'?[0,.64,1]:[0,.5,1])[index]!;cancelPlay=animateValue({from:progress,to:target,duration:1000,onUpdate:value=>{progress=value;draw();}});});steps.append(button);});
...
document.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(button=>button.addEventListener('click',()=>{
  const target=button.dataset.kind as CellKind;stop();cancelSwitch();scene.style.opacity='1';if(target===kind)return;
  let changed=false;
