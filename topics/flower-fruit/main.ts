...
function cherry(mature: boolean) {
  ...
  const scale = mature ? 1 : .66;
  return `<g transform="translate(284 290) scale(${scale})" filter="url(#botanical-shadow)"><use href="#cherry-outline" fill="url(#${mature ? 'cherry-skin' : 'young-fruit'})"/><g clip-path="url(#cherry-mask)">${freckles}<path d="M-48 -49 C-61 -35 -65 -13 -59 7" stroke="#fff8e6" stroke-width="12" stroke-linecap="round" opacity=".43" fill="none"/><ellipse cx="-40" cy="-52" rx="13" ry="7" transform="rotate(-25 -40 -52)" fill="#fffbed" opacity=".66"/><path d="M10 -68 Q27 -14 10 76" stroke="${mature ? '#602233' : '#71894d'}" stroke-width="1.4" fill="none" opacity=".35"/></g><ellipse cx="6" cy="-67" rx="11" ry="4" fill="${mature ? '#4e3f31' : '#647649'}"/><path d="M5 -66 Q21 -108 12 -139" fill="none" stroke="url(#stalk)" stroke-width="6" stroke-linecap="round"/></g>`;
}
let stage=0;
el('pollinate').addEventListener('click',()=>{stage=1;draw()});
el('next').addEventListener('click',()=>{if(stage>0)stage=Math.min(3,stage+1);draw()});
el('reset').addEventListener('click',()=>{stage=0;draw()});
function draw() {
  ...
  let specimen = `<path d="M-22 122 Q125 143 223 140 T469 109" stroke="url(#bark)" stroke-width="17" fill="none" stroke-linecap="round"/><path d="M-18 118 Q121 137 221 136 T465 105" stroke="#d2b58e" stroke-width="2.1" fill="none" opacity=".4"/>${barkMarks}<path d="M295 137 Q299 169 ${stage < 2 ? 282 : stage === 2 ? 292 : 296} ${stage < 2 ? 252 : stage === 2 ? 198 : 151}" fill="none" stroke="url(#stalk)" stroke-width="5"/>` + orchardLeaf(351,131,.8,-27) + orchardLeaf(171,143,.8,143);
  if (stage < 2) specimen += blossom();
  else specimen += cherry(stage === 3);
  if (stage === 2) specimen += `<g fill="#d0aaa0" stroke="#ac887b" stroke-width=".8" opacity=".75"><path d="M238 299 Q211 331 225 355 Q246 342 238 299Z"/><path d="M319 322 Q344 337 345 363 Q324 355 319 322Z"/></g>`;
  if (stage === 1) specimen += bee() + `<g fill="#d6a143"><circle cx="275" cy="246" r="2.8"/><circle cx="280" cy="251" r="2"/><circle cx="270" cy="250" r="1.8"/></g>`;
  let inset = `<rect x="485" y="143" width="232" height="306" rx="29" fill="#fffdf4" fill-opacity=".92" stroke="#d9dfca" filter="url(#inset-shadow)"/>` + label(601,176,stage<2?t('花心剖面'):t('果实剖面'),'inset-heading');
  if (stage < 2) {
    ...
    if (stage === 1) inset += `<g fill="#ddad55" stroke="#b8843c" stroke-width=".8"><circle cx="599" cy="204" r="6"/><circle cx="610" cy="202" r="4"/></g><path class="pollen-tube" pathLength="1" d="M599 210 Q607 248 600 285 C593 310 607 320 603 336" stroke="#cc8651" stroke-width="3" stroke-linecap="round" fill="none"/>`;
    inset += label(601,414,t('子房包着胚珠'),'small');
  } else {
    const flesh = stage === 3 ? 'url(#flesh)' : '#d7dfac';
    inset += `<g transform="translate(601 295) scale(.85)"><use href="#cherry-outline" fill="${flesh}" stroke="${stage === 3 ? '#ac4257' : '#7f9758'}" stroke-width="6"/><path d="M-58 -45 Q-82 6 -45 52" fill="none" stroke="#fff3d1" stroke-width="4" opacity=".45"/><path d="M-11 -37 C-38 -21 -36 37 -9 53 C13 61 38 26 29 -7 C22 -30 3 -45 -11 -37Z" fill="url(#stone)" stroke="#947248" stroke-width="2"/><path d="M-11 -29 Q-28 0 -14 42 M0 -34 Q-13 4 1 45 M16 -21 Q9 12 14 30" fill="none" stroke="#765334" stroke-width="1.1" opacity=".45"/><path d="M-1 -18 C-14 -7 -11 24 1 29 C14 21 17 -4 -1 -18Z" fill="#f2dfb3" stroke="#aa8959" stroke-width="1.1"/><path d="M0 -12 Q-4 2 1 23" fill="none" stroke="#cfb783" stroke-width="1"/></g>` + label(601,414,t('果核里面有种子'),'small');
  }
  const titles=[t('花开了'),t('花粉到达花心'),t('小果实开始长大'),t('樱桃成熟了')];
  scene.innerHTML = defs + backdrop + `<g class="specimen stage-enter">${specimen}</g>` + `<g class="cutaway stage-enter">${inset}</g><rect x="125" y="468" width="510" height="37" rx="18" fill="#fdfcef" fill-opacity=".79" stroke="#d7ddc7"/>` + label(380,493,titles[stage]!, 'stage-caption');
