...
let stage=0,shown=0,playing=false,last=0,frame=0;
let closeView=false,camera=[0,0,760,530];
let cancelCamera:()=>void=()=>{};
el('zoom').addEventListener('click',()=>{
  cancelCamera();closeView=!closeView;scene.parentElement?.classList.toggle('detail-view',closeView);const from=[...camera],to=closeView?[469,133,264,326]:[0,0,760,530];
  el('zoom').textContent=closeView?t('返回整体'):t('放大剖面');el('zoom').setAttribute('aria-pressed',String(closeView));
  cancelCamera=animateValue({from:0,to:1,duration:650,onUpdate:p=>{camera=from.map((v,i)=>v+(to[i]!-v)*p);scene.setAttribute('viewBox',camera.join(' '));}});
});
let cancelGrowth: () => void = () => {};
const reduced = window.matchMedia('(prefers-reduced-motion: reduce)');
function stop() { playing=false; cancelAnimationFrame(frame); frame=0; cancelGrowth(); }
function seek(target:number) {
  stop(); stage=target;
  cancelGrowth=animateValue({from:shown,to:target,duration:1800,onUpdate:value=>{shown=value;draw();}});
}
function tick(now:number) {
  if (!playing) return;
  shown=Math.min(3,shown+Math.min(.1,(now-last)/1000)/8); last=now;
  stage=Math.floor(shown); if(shown===3)playing=false;
  draw(); if(playing)frame=requestAnimationFrame(tick);
}
el('pollinate').addEventListener('click',()=>seek(1));
el('next').addEventListener('click',()=>{if(shown>=1)seek(Math.min(3,Math.floor(shown)+1));});
el('reset').addEventListener('click',()=>seek(0));
el('journey-play').addEventListener('click',()=>{
  if(playing){stop();draw();return;}
  cancelGrowth(); if(shown>=3)shown=0;
  if(reduced.matches){seek(Math.min(3,Math.floor(shown)+1));return;}
  playing=true;last=performance.now();draw();frame=requestAnimationFrame(tick);
});
el<HTMLInputElement>('journey').addEventListener('input',()=>{stop();shown=Number(el<HTMLInputElement>('journey').value)/1000;stage=Math.floor(shown);draw();});
const visibility=()=>{if(document.hidden){stop();draw();}};
document.addEventListener('visibilitychange',visibility);
reduced.addEventListener('change',()=>{stop();draw();});
window.addEventListener('pagehide',()=>{stop();cancelCamera();});

function draw() {
  const pollination=pollinationFrame(shown);
  ...
  if (shown<1.35) specimen += `<g opacity="${1-Math.max(0,(shown-1)*3)}">${bee(pollination.beeX,pollination.beeY)}</g>`;
  if (shown<2) specimen += `<g fill="#d6a143" stroke="#a77933" stroke-width=".6" opacity="${1-Math.max(0,shown-1)}"><circle cx="${pollination.pollenX}" cy="${pollination.pollenY}" r="3.1"/><circle cx="${pollination.pollenX-4}" cy="${pollination.pollenY+2}" r="2"/></g>`;
  ...
  if (sectionBlend < 1) {
    ...
    if (pollination.deposited) inset += `<g fill="#ddad55" stroke="#b8843c" stroke-width=".8"><circle cx="599" cy="204" r="6"/><circle cx="610" cy="202" r="4"/></g><path pathLength="1" stroke-dasharray="1" stroke-dashoffset="${1-pollination.tube}" d="M599 210 Q607 248 600 285 C593 310 607 320 603 336" stroke="#cc8651" stroke-width="3" stroke-linecap="round" fill="none"/>`;
  ...
  scene.innerHTML = defs + backdrop + `<g class="specimen">${specimen}</g>` + `<g class="cutaway">${inset}</g><rect x="125" y="468" width="510" height="37" rx="18" fill="#fdfcef" fill-opacity=".79" stroke="#d7ddc7"/>` + label(380,493,titles[Math.floor(shown)]!, 'stage-caption');
result.textContent=shown<1?(pollination.deposited?t('花粉先落到柱头，再长出花粉管。它不是整粒钻进子房；花粉管把精细胞送向胚珠。'):t('蜜蜂身上的花粉随它一起靠近；只有接触柱头后，花粉才留在花上。')):[t('花里藏着子房，子房里面有胚珠。先让蜜蜂带来合适的花粉。'),t('花粉到了柱头上，这叫授粉。接下来，花粉管生长，帮助完成受精。'),t('受精后，胚珠发育成种子，子房逐渐发育成果实。花瓣会慢慢凋落。'),t('成熟樱桃的果肉和硬果核来自子房壁；种子藏在硬果核里面。不是花瓣变成了樱桃。')][Math.floor(shown)]!;
  el<HTMLButtonElement>('next').disabled=shown<1||shown>=3;
  el<HTMLInputElement>('journey').value=String(Math.round(shown*1000));
  el('journey-play').textContent=playing?t('暂停'):reduced.matches?t('看下一步'):shown>=3?t('再看一次'):t('连续观察');
  el<HTMLButtonElement>('pollinate').disabled=shown>0;
}
draw();
