...
function draw(phase=0,envelope=1){const a=value('amplitude')*envelope;el('string').setAttribute('d',Array.from({length:51},(_,i)=>{const x=172+i/50*556,y=230+a*Math.sin(Math.PI*i/50)*Math.sin(phase);return `${i?'L':'M'}${x} ${y}`;}).join(''));el('pitch').textContent=t('每秒振动约 {{frequency}} 次',{frequency:Math.round(frequency(value('tension')))});el('air').setAttribute('d',envelope<.05?'':`M790 175Q825 230 790 285M811 154Q860 230 811 306`);el('air').setAttribute('opacity',String(Math.abs(Math.sin(phase))*envelope));}
function halt(){serial++;cancelAnimationFrame(frame);frame=0;if(osc){try{osc.stop();}catch{/* already ended */}osc.disconnect();osc=undefined;}draw(0,0);}
...
el('pluck').addEventListener('click',()=>{void pluck();});el('stop').addEventListener('click',()=>{halt();el('readout').textContent=t('振动停止了，声音也停了。');});for(const id of ['tension','amplitude'])el(id).addEventListener('input',()=>{halt();draw(value('phase')/100*Math.PI*2);el('readout').textContent=t('先拨一下；绷紧程度改变音高，拨动幅度改变响轻。');});el('phase').addEventListener('input',()=>{halt();draw(value('phase')/100*Math.PI*2);el('readout').textContent=t('这是手动慢镜头：拖动滑块，看橡皮筋怎样来回运动。');});document.addEventListener('visibilitychange',()=>{if(document.hidden){halt();void audio?.suspend();}});window.addEventListener('pagehide',event=>{halt();if(event.persisted)void audio?.suspend();else void audio?.close();});draw(0,0);el('readout').textContent=t('先拨一下；绷紧程度改变音高，拨动幅度改变响轻。');
