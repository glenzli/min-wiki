...
function update() {
  ...
  el('metric').textContent=result.cloud===0?t('水汽尚未凝结'):progress<.67?t('云滴还在长大'):progress<.98?t('雨滴正在下落'):result.reachesGround?t('雨滴落到地面'):t('雨滴在空中蒸发');
  el('humidity-value').textContent=value('humidity')+'%';
  el('science-live').textContent=result.cloud===0?t('水汽尚未凝结'):t('半径：{{radius}} mm → {{remaining}} mm；云下湿度：{{humidity}}%\n假设云滴半径 0.01 mm，初始雨滴约含 {{count}} 个云滴的水量。',{radius:result.radiusMm.toFixed(2),remaining:result.remainingRadiusMm.toFixed(2),humidity:settings.humidity,count:Math.round(equivalentCloudDrops(.7)).toLocaleString()});
