...
function update() {
  ...
  el('scene-title').textContent = view === 'plant' ? t('四条羽片，许多对小叶') : t('叶枕：叶片基部的运动器官');
  el('water-value').textContent = `${Math.round(s.water * 100)}%`; el('water-bar').style.width = `${s.water * 100}%`;
  ...
  el('science-live').textContent = t('当前阶段：{{stage}}\n观察位置：{{pinna}}\n含水趋势：{{water}}%（归一化示意）', { stage: steps[s.stage], pinna: select('pinna').selectedOptions[0].text, water: Math.round(s.water * 100) });
  ...
  scene?.draw(progress, config, view);
}
...
el('steps').replaceChildren(...steps.map((label, i) => { const b = document.createElement('button'); b.textContent = label; b.addEventListener('click', () => { stop(); progress = [0, .21, .43, .62, .84][i]; update(); }); return b; }));
