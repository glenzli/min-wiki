...
import { StellarGas } from '../rendering/stellarGas.js';
import { DisruptionLibrary } from './disruptionLibrary.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { createBlackHole, createStar } from '../shaders/blackHoleShader.js';
import { orbitAt, clamp, smooth, randomSource, TIDAL_RADIUS, TAU, SCENARIOS } from './encounter.js';
...
export class TdeSimulation {
  constructor(container) {
    ...
    this.renderer.domElement.setAttribute('aria-label', '恒星经过黑洞的三维动画，故事和播放控制位于画面外');
  ...
  selectScenario(scenario) {
    ...
    this.renderer.domElement.setAttribute('aria-label', SCENARIOS[scenario].noBlackHole ? '无黑洞对照：完整恒星沿直线匀速运动' : '恒星经过黑洞的三维动画，故事和播放控制位于画面外');
    ...
    for (const child of [...this.arrows.children]) {
      child.geometry.dispose(); child.material.dispose(); this.arrows.remove(child);
    }
