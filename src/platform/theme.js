...
export const themePreference = value => ['auto', 'light', 'dark'].includes(value) ? value : 'auto';
export function resolveTheme(preference, contentTheme = 'light') {
...
export function setContentTheme(theme) {
  document.documentElement.dataset.contentTheme = theme === 'dark' ? 'dark' : 'light';
  applyTheme();
}
export function mountThemeControl(host) {
  const label = document.createElement('label');
  label.className = 'theme-control';
  const text = document.createElement('span'); text.textContent = '外观';
  const select = document.createElement('select'); select.dataset.themeSelect = ''; select.setAttribute('aria-label', '页面外观');
  for (const [value, title] of [['auto','跟随内容'], ['light','浅色'], ['dark','深色']]) {
